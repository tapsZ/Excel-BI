...
`H2:H31` means "every cell from H2 down to H31". The colon means "through". You can type it, or just drag over the cells with the mouse after typing `=SUM(`.

### The five you will use constantly

| Function | Answers | Example |
|---|---|---|
| `SUM` | What is the total? | `=SUM(H2:H31)` |
| `AVERAGE` | What is the typical value? | `=AVERAGE(H2:H31)` |
| `MAX` | What is the biggest? | `=MAX(H2:H31)` |
| `MIN` | What is the smallest? | `=MIN(H2:H31)` |
| `COUNT` | How many numbers are there? | `=COUNT(H2:H31)` |
...
```
=COUNTIF(F2:F31,"Laptop")        how many orders were laptops
=COUNTIF(H2:H31,">1000")         how many orders exceeded $1,000
```

**`SUMIF(range, criteria, sum_range)`** — add up only the matching rows.

```
=SUMIF(F2:F31,"Laptop",H2:H31)
```

Read it left to right: *look through F2:F31; wherever you find "Laptop", add the matching value from H2:H31.*

The two ranges must be the same height and line up row for row. `F2:F31` and `H2:H31` both cover rows 2–31, so row 5's category is checked against row 5's sales figure.
...
| Cell | Formula | Idea |
|---|---|---|
| `L2` | `=SUM(H2:H31)` | total sales |
| `L3` | `=AVERAGE(H2:H31)` | typical order |
| `L4` | `=MAX(H2:H31)` | biggest order |
| `L5` | `=MIN(H2:H31)` | smallest order |
| `L6` | `=COUNT(H2:H31)` | how many orders |
| `L7` | `=SUM(G2:G31)` | total units — a different column |
...
2. In `=SUMIF(F2:F31,"Laptop",H2:H31)`, what is each of the three arguments for?
3. Why must `=IF(H2>=1000,"Large",IF(H2>=300,"Medium","Small"))` test 1000 before 300?
4. Write a formula counting orders **over** $500 in `H2:H31`.
